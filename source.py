...
class FrozenLake(FrozenLakeEnv):
    ...
    def _calculate_transition_prob(self, position, action):
        ...
        current_state = np.ravel_multi_index(position, self.shape)
        ...
        if 0 <= new_row < self.shape[0] and 0 <= new_col < self.shape[1]:
            ...
            if self._lake[new_row, new_col]:  # If entering a hole
                return [(1.0, new_state, -1, True)]  # Falling in hole leads to loss
            else:
                return [(1.0, new_state, 0, False)]  # Safe move
        else:
            return [(1.0, current_state, 0, False)]  # Invalid move
        ...
        while frontier:
            ...
            if not (r, c) in discovered:
                ...
                for x, y in directions:
                    ...
                    if (
                        r_new < 0
                        or r_new >= self.shape[0]
                        or c_new < 0
                        or c_new >= self.shape[1]
                    ):
                        continue
    ...
    def _render_gui(self, mode):
        ...
        if self.hole_img is None:
            self.hole_img = pygame.transform.scale(
                pygame.image.load(path.join(image_path, "img/hole.png")), self.cell_size
            )

        if self.ice_img is None:
            self.ice_img = pygame.transform.scale(
                pygame.image.load(path.join(image_path, "img/ice.png")), self.cell_size
            )

        if self.goal_img is None:
            self.goal_img = pygame.transform.scale(
                pygame.image.load(path.join(image_path, "img/goal.png")), self.cell_size
            )
        ...
        for s in range(self.nS):
            ...
            if s == self.start_state:  # Render starting position
                self.window_surface.blit(
                    self.start_img, pos
                )  # Optional starting position image
            ...
            if s == self.s:  # Render player (elf or froggie)
                elf_pos = (
                    pos[0],
                    pos[1] - 0.1 * self.cell_size[1],
                )  # Small adjustment for positioning
                last_action = (
                    self.lastaction if self.lastaction is not None else 2
                )  # Default to down
